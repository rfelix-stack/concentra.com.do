    ...
    <div class="mx-auto grid max-w-7xl grid-cols-1 lg:grid-cols-2">
      <div class="relative px-6 pt-24 pb-20 sm:pt-32 lg:static lg:px-8 lg:py-48">
        <div class="mx-auto max-w-xl lg:mx-0 lg:max-w-lg">
          <div class="absolute inset-y-0 left-0 -z-10 w-full overflow-hidden bg-primary-50 ring-1 ring-primary-100 lg:w-1/2">
            <svg class="absolute inset-0 size-full [mask-image:radial-gradient(100%_100%_at_top_right,white,transparent)] stroke-primary-100" aria-hidden="true">
              <defs>
          ...
          <img :src="items[currentIndex]?.image" class="h-12 w-auto object-cover" v-motion-fadein-up-once alt="" />
        ...
        <div class="mx-auto max-w-xl lg:mr-0 lg:max-w-lg">
          ...
          <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
            <div class="sm:col-span-2" v-if="items.length">
              ...
              <div class="mt-2.5">
                <select id="item-select" name="item-select" v-model="selectedSlug" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-secondary outline-1 -outline-offset-1 outline-primary-100 focus:outline-2 focus:-outline-offset-2 focus:outline-primary">
            ...
            <div>
              ...
              <div class="mt-2.5">
                <input v-model="form.firstName" required type="text" name="first-name" id="first-name" autocomplete="given-name" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-secondary outline-1 -outline-offset-1 outline-primary-100 placeholder:text-gris-aluminio focus:outline-2 focus:-outline-offset-2 focus:outline-primary" />
              </div>
            </div>
            <div>
              <label for="last-name" class="block text-sm/6 font-semibold text-secondary">Apellidos</label>
              <div class="mt-2.5">
                <input v-model="form.lastName" required type="text" name="last-name" id="last-name" autocomplete="family-name" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-secondary outline-1 -outline-offset-1 outline-primary-100 placeholder:text-gris-aluminio focus:outline-2 focus:-outline-offset-2 focus:outline-primary" />
              </div>
            </div>
            <div class="sm:col-span-2">
              <label for="email" class="block text-sm/6 font-semibold text-secondary">Email</label>
              <div class="mt-2.5">
                <input v-model="form.email" required type="email" name="email" id="email" autocomplete="email" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-secondary outline-1 -outline-offset-1 outline-primary-100 placeholder:text-gris-aluminio focus:outline-2 focus:-outline-offset-2 focus:outline-primary" />
              </div>
            </div>
            <div>
              <label for="phone" class="block text-sm/6 font-semibold text-secondary">Teléfono</label>
              <div class="mt-2.5">
                <input v-model="form.phone" type="text" name="phone" id="phone" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-secondary outline-1 -outline-offset-1 outline-primary-100 placeholder:text-gris-aluminio focus:outline-2 focus:-outline-offset-2 focus:outline-primary" />
              </div>
            </div>
            <div>
              <label for="company" class="block text-sm/6 font-semibold text-secondary">Empresa</label>
              <div class="mt-2.5">
                <input v-model="form.company" type="text" name="company" id="company" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-secondary outline-1 -outline-offset-1 outline-primary-100 placeholder:text-gris-aluminio focus:outline-2 focus:-outline-offset-2 focus:outline-primary" />
              </div>
            </div>
            <div class="sm:col-span-2">
              <label for="message" class="block text-sm/6 font-semibold text-secondary">Mensaje</label>
              <div class="mt-2.5">
                <textarea v-model="form.message" name="message" id="message" rows="4" class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-secondary outline-1 -outline-offset-1 outline-primary-100 placeholder:text-gris-aluminio focus:outline-2 focus:-outline-offset-2 focus:outline-primary"></textarea>
              </div>
            </div>
          </div>
          <div class="mt-8 flex justify-end">
            <button type="submit" :disabled="submitting" class="rounded-full bg-primary px-3.5 py-2.5 text-sm font-light text-white shadow-xs hover:bg-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-secondary transition-colors duration-300 ease-in-out disabled:opacity-60">
...
const mapRows = (rows) => (rows || []).map((s) => ({
  label: s.name,
  slug: s.slug,
  image: directusAsset(s.isotipo, { format: 'webp', width: 320, height: 160, fit: 'cover' }),
  intro: s.intro || ''
}))
